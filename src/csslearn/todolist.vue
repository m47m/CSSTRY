		...
		<div class="container">
			...
			<div class="form-field">
				...
				<form class="form-wrapper">
					...
					<button type="button" class="submit-btn submit-btn1"  @click="submit()">
						<span>Submit</span>
					</button>
				</form>
			</div>
...
var btn,input,text,hidden;

onMounted(()=>{
	btn = document.querySelector(".submit-btn");
	input = document.getElementsByTagName("input")[0];
  text = document.getElementsByClassName("empty-todos")[0];
  hidden = document.getElementsByClassName("hidden")[0];

	console.log(btn,input,text,hidden)
...
const submit = function(){
		...
		if(input.value.trim()){
			...
			localData.push({text:input.value,done:false,date:getDate()});
...
const load = function(){
	...
	data.forEach((element,index) => {
      ...
      //     str;
			...
			str = "×	"+element.text + "  "+index+"  "+str;
		}else{
			// str =
      //     '<li class="todo-item1"><input type="checkbox" class="left-icon"></input><div class="todo-text">' +
      //     element.text +
      //     '</div><div class="right-icon" id="' +
      //     index +
...
const fil = function(){
	...
	if(document.querySelector(".active.span")){
		arr = data.filter((item) => item.data == false)
...
const updata = function(){
	let left = document.querySelectorAll(".left-icon");

	left.forEach((element) =>{
		element.addEventListener('click',()=>{
			let data = getData();
			let index = element.parentNode.children[2].getAttribute("id");
